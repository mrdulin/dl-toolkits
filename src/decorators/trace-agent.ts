function traceDecoratorFactory(options?: { name: string }) {
  ...
  return function traceClassDecorator<T extends new (...args: any[]) => {}>(constructor: T) {
    ...
    for (const instanceMethod of instanceMethods) {
      ...
      constructor.prototype[instanceMethod] = async (...args: any[]) => {
        ...
        childSpan.addLabel('returnValue', rval);
        childSpan.endSpan();
        return rval;
      };
    }
...
export { traceDecoratorFactory as trace, traceClassDecoratorFactory, traceStaticClassDecoratorFactory };
